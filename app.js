...
function initNavbar() {
  ...
  trigger?.addEventListener('focus', showDD);
  trigger?.addEventListener('blur', () => hideDD(350));
  trigger?.addEventListener('keydown', e => {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); dropdown?.classList.toggle('open'); }
    if (e.key === 'Escape') hideDD(0);
  });
...
function createSlider(containerEl, items, opts = {}) {
  ...
  /* ── Measure actual card step width (card + gap) dynamically ── */
  let cardW = 241; /* fallback */
  const measureCardW = () => {
    const firstCard = track.querySelector('.pcard');
    if (firstCard) {
      const style = getComputedStyle(track);
      const gap   = parseFloat(style.gap || style.columnGap || '17.6');
      cardW = firstCard.getBoundingClientRect().width + gap;
    }
  };
  /* Measure after paint */
  requestAnimationFrame(() => { requestAnimationFrame(measureCardW); });

  const loopW   = () => items.length * cardW;  /* width of one full set (dynamic) */
  ...
  const normalizeOffset = (animated = false) => {
    const lw = loopW();
    if (offset >= lw) {
      offset -= lw;
  ...
  containerEl.addEventListener('touchmove', e => {
    ...
    const lw    = loopW();
    let   live  = ((swipeOffset + delta) % lw + lw) % lw;
    track.style.transition = 'none';
  ...
  containerEl.addEventListener('touchend', e => {
    ...
    if (Math.abs(delta) > 40) {
      ...
      offset = delta > 0
        ? ((swipeOffset + steps * cardW) % lw + lw) % lw
        : ((swipeOffset - steps * cardW) % lw + lw) % lw;
    } else {
      offset = swipeOffset;
    }
